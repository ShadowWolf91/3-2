...
struct NTP_packet
{
	...
	DWORD32		RootDispersion;
	CHAR		ReferenceIdentifier[4]; //идентификатор
	DWORD		ReferenceTimestamp[2]; //32 бита - секунды с 01.01.1900 
	DWORD64		OriginateTimestamp;
...
{
	// Обратите внимание, что LONGLONG — это 64-битное значение.
	LONGLONG ll;
...
int main()
{
	int h = CLOCKS_PER_SEC; 
	clock_t t = clock();
	int d = 613608 * 3600; //сек между 1900 - 1970
	time_t ttime; //кол-во сек с 1970 
	time(&ttime);
